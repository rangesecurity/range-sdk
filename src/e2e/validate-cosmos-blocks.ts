/**
 * Strict Zod validation for Cosmos block JSON files (Range-wrapped format).
 *
 * Usage:
 *   node dist/e2e/validate-cosmos-blocks.js [block-dir]
 */

import { readFileSync, readdirSync } from 'fs';
import * as path from 'path';
import { CosmosBlockVal } from '../types/ICosmosBlock';

// ── Run validation ──

const DEFAULT_DIR = path.join(process.cwd(), 'src', 'test_data', 'osmosis-1');
...
console.log(`Validating ${files.length} Cosmos blocks from ${blockDir}\n`);
...
for (const file of files) {
  ...
  const result = CosmosBlockVal.strict().safeParse(json);
...
if (allIssues.length === 0) {
  console.log(`All ${files.length} Cosmos blocks passed strict validation.`);
  process.exit(0);
}
