/**
 * Strict Zod validation for Solana block JSON files (Range-wrapped format).
 *
 * Usage:
 *   node dist/e2e/validate-blocks.js [block-dir]
 */

import { readFileSync, readdirSync } from 'fs';
import * as path from 'path';
import { SolanaBlockVal } from '../types/ISolanaBlock';

// ── Run validation ──

const DEFAULT_DIR = path.join(process.cwd(), 'src', 'test_data', 'solana');
...
console.log(`Validating ${files.length} Solana blocks from ${blockDir}\n`);
...
for (const file of files) {
  ...
  const result = SolanaBlockVal.strict().safeParse(json);
...
if (allIssues.length === 0) {
  console.log(`All ${files.length} Solana blocks passed strict validation.`);
  process.exit(0);
}
